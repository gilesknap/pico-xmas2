...
class LcdApi:
    ...
    async def async_putstr(self, string, x, y):
        # Do individual writes to the LCD asynchronously.
        # The local x,y avoid clashes when multiple coroutines are running
        # and writing to different lines
        for char in string:
            await self.async_putchar(char, x, y)
            x += 1
